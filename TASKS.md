...
| 2026-08-19 | bench window 3 | agent running | `task/bench-window-3` | 02:30: llama-server STOPPED (restore after: `launchctl bootstrap gui/501 ~/Library/LaunchAgents/com.artur.llama-server.plist`). Docket: A) FAST_ATTN qwen3:8b incl. long-ctx (T4.8 final go/no-go), B) T4.10 length-vs-chunking on real gpt-oss, C) 1.69 tok/s attribution — methodology audit FIRST (harness GB/s may be the artifact), real bytes/token, top-3 time sinks, one beam run, D) headline stability check. |
