...
| 2026-08-18 | T1.8b | agent running | `task/T1.8b-tuned-attn` | wave 4: tuned Metal decode-attn kernel behind FAST_ATTN=1 (the remaining ~2x); STOP = best-attempt + limiter analysis |
| 2026-08-18 | T4.2 | agent running | `task/T4.2-q4k-alu` | wave 4: Q4_K ALU profile + ≤2 attempts, else Q4K_ANALYSIS.md |
| 2026-08-18 | T1.9 | agent running | `task/T1.9-stream-load` | wave 4: per-tensor GGUF staging; peak RSS ≈ model size; CPU-validated (METAL full numbers at bench window) |
| 2026-08-18 | T2.5 | agent running | `task/T2.5-sync-amortize` | wave 4: drain-every-N tokens (N≤4), device-side chaining if jit allows |
| 2026-08-18 | T4.5 | agent running | `task/T4.5-force-realize` | wave 4: force-realize API shared by all loaders + stranded-param guard |
