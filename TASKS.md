...
| 2026-08-25 | T4.19 (new) | open | — | **Split-placement divergence at depth:** both split rows diverge from the (mutually byte-identical) 4 single-device configs at decode token 60 under 512/128; T4.18's byte-identical proof was at 16/64. Suspect the known cross-implementation FP-drift class (accumulation order changes at the boundary) — but prove it: tiny-config bisection (which hop, which dtype), logit-delta magnitude at divergence, and whether fp16-KV/fp32-state interacts. Not a crash; blocks nothing; needed before any "byte-identical pooling" claim ships in TD.4 material. `[DOCK]`, Sonnet-able. |
