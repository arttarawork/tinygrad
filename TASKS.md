...
| 2026-08-19 | bench window 4 | agent running | `task/bench-window-4` | 09:49: llama-server STOPPED (restore: `launchctl bootstrap gui/501 ~/Library/LaunchAgents/com.artur.llama-server.plist`). A) gpt-oss post-T4.13 confirm (decode/bytes/parity/beam), B) headline stability post waves 7-8 (jit-key + ops.py changes), C) optional gpt-oss long-ctx. |
