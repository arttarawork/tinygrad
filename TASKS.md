...
- **T4.101 🔄 — the ~12 KB/token of non-KV context memory** `[MAC]` deps: T4.100 (agent, 09-11): the card's usage grows ~30 KB/token at int4 KV and ~46 at int8 (chain 14/16 probes) while the KV cache itself is 18 / 35 KB/token. Find every buffer that scales with max_context (candidates: `precompute_freqs_cis(rope_dim=64, max_context)` allocated per block instance at model.py:722/775 — 256 B/token per copy, 65 copies would be 17 KB/token; the MTP block's own caches; position/mask buffers), quantify for the 27B geometry (65 blocks, 16 attention, 4 KV heads × 256, rope_dim 64, nextn 1), share what can be shared (one rope table per device), test on CPU with tiny configs. Payoff: every int8 ceiling up by ~a third.
