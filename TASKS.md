...
| 2026-08-19 | fork CI red (PR #1) | agent running | `task/ci-green-cpu` | Artur pushed integration/wave1 + opened fork PR #1 → 13 failed checks. Triaged: 10 = fail-fast cascade; 3 real, ALL reproduce locally with **DEV=CPU** (CI's Linux default — our gates always ran METAL-default, a real gate gap). Classes: (1) tuned-attn kernel/tests not device-gated (gpudims IndexError; FAST_ATTN=1 would crash prod on CPU — wrapper gets has_local fallback); (2) **the device_map "flake" SOLVED: DEV=CPU makes ref/split emit different tokens — suspected shared-unrealized-randn between models (verify-first)**; (3) gptoss byte test Context misuse TypeError. MacOS(unit) CI failure likely same tests. Gate lesson: add a DEV=CPU pass to our local gate list. |
