...
- **T4.103 📋 — flash-style tiled prefill attention** `[MAC+dock]` deps: T4.101: replace the full (H, T, max_context) fp32 score/softmax rows of a prefill chunk with an online-softmax loop over KV tiles (and the same for the MTP head's prefill); removes the ~1.6 GB score arena at 262k (~10 KB/token) and probably speeds prefill (the arena is written and read back through memory). Numerics must match the current path (flip-rate check).
