...
- **T4.103a 🔄 — head-grouped prefill attention (arena ÷ n_kv_heads)** `[MAC+dock]` deps: T4.101 (agent, 09-11 19:45): compute the score pipeline per KV-head group (24 q heads = 4 groups of 6, one k/v head each, `SDPA_HEAD_GROUPS`, default n_kv_heads, 1 = today) instead of all heads at once — the planned arena drops ~4× (~6.8 → ~1.7 KB/token, ~1.6 → ~0.4 GB at 262k) with the same numerics and the symbolic-length kernels intact (they still loop over start_pos+T). Device check: prefill tok/s (a few more launches), flip rate, and the int8 ceilings re-probed.
