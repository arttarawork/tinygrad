...
compile-server bug found+fixed en route (prime PR candidate). **TD.3 pooling rehearsal PASSED
same night: METAL+NV split byte-identical, hop ~80-90 µs (8-9x under the CPU floor), both
islands graph — pooling overhead is ~free.** Next: TD.3 remainder (T2.1/T2.2/T2.3 validation,
T3.3 MoE-on-dock, BEAM'd pooling rows), T4.15/T4.16 filler, PR-train route decision (incl. T4.14).
