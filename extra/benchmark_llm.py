...
if __name__ == "__main__":
  ...
  print(f"prefill {args.prompt_tokens/(pt-st):.3f} tok/s {GlobalCounters.global_mem/(pt-st)/1e9:.2f} GB/s", flush=True)

  GlobalCounters.reset()
  for _ in range(args.decode_tokens): output.append(next(gen))
  et = time.perf_counter()
  print(f"decode {args.decode_tokens/(et-pt):.3f} tok/s {GlobalCounters.global_mem/(et-pt)/1e9:.2f} GB/s output {output}", flush=True)
