"""T4.107a: validate + time the NV split-KV decode attention (kernels/nv_attn.py) on a real GGUF, on the card.
...
@function/JIT trace does not re-key on NV_CUSTOM_ATTN), so the paths cannot be flipped inside one process:
  DEV=NV KV_INT4=1 NV_CUSTOM_QUANT=1 GDN_NV_FUSED_DECODE=1 KQUANT_STAGE=0 NV_CUSTOM_ATTN=0 PYTHONPATH=. python extra/nv_attn_validate_real.py run \
    --model /Users/artur/models/qwen3.8-27b-q4/Qwen3.8-27B-Q4_0.gguf --max-context 131072 --fill 2048,20480,61440 --out generic.npz
...
def run(args:argparse.Namespace) -> None:
  ...
  assert max(fills) + args.steps < args.max_context, "fill + steps must fit max_context"
...
def compare(args:argparse.Namespace) -> None:
  ...
  print(f"{'fill':>8} {'A ms/step':>10} {'B ms/step':>10} {'speedup':>8}  argmax  gap A / gap B")
  for key in sorted(a.files, key=int):
    ma, mb = a[key], b[key]
    agree = 'same' if int(ma[2]) == int(mb[2]) else 'DIFF'
    print(f"{int(key):8d} {ma[0]:10.1f} {mb[0]:10.1f} {ma[0] / mb[0]:8.2f}x  {agree:5s}  {ma[3]:.4f} / {mb[3]:.4f}")
...
def main() -> None:
  ...
  r.add_argument("--chunk-size", type=int, default=512)
