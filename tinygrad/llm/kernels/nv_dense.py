...
def nv_f16_gemv(layer:Linear, x:Tensor) -> Tensor|None:
  ...
  if layer.dense_weight is None:
    # realize once: casting+copying a lazy (ggml-load-chain-or-not) weight view on every decode step would cost
    # as much as the matmul it replaces. Always fp16 regardless of the original dtype -- fp32 weights lose
    # precision here, the same trade the quantized paths already make
    layer.dense_weight = layer.weight.cast(dtypes.float16).contiguous().realize()
