...
def causal_mask(pos:Tensor, T:int|UOp, start_pos:int|UOp, dtype:DType, sliding_window:int=0) -> Tensor:
  ...
  cost of the pooled qwen3.6-35B). Callers pass a buffer of 2*max_context entries: the SYMBOLIC bound of start_pos+T is
  (max_context-1)+chunk_size, past the cache, and CHECK_OOB's z3 check (on in CI) rejects a load it cannot prove in-bounds even
  though generate() never runs there; a buffer that covers the bound is 512 KB at 64k context."""
...
class TransformerBlock(FFNBlock):
  ...
  def _attention(self, x:Tensor, start_pos:int|UOp) -> Tensor:
    ...
    mask = causal_mask(positions(2*self.config.max_context, x.device), T, start_pos, x.dtype, self.config.sliding_window) \
  ...
  def _init_state(self, x:Tensor):
    if not hasattr(self, "cache_kv"):
      positions(2*self.config.max_context, x.device)  # realize the shared position buffer here, outside any JIT capture (see causal_mask)
...
class MLATransformerBlock(FFNBlock):
  ...
  def _attention(self, x:Tensor, start_pos:int|UOp) -> Tensor:
    ...
    mask = causal_mask(positions(2*self.config.max_context, x.device), T, start_pos, x.dtype) if resolve(T != 1) else None
  ...
  def _init_state(self, x:Tensor):
    if not hasattr(self, "cache_k"):
      positions(2*self.config.max_context, x.device)
