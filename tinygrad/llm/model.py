...
def causal_mask(pos:Tensor, T:int|UOp, start_pos:int|UOp, dtype:DType, sliding_window:int=0) -> Tensor:
  ...
  cost of the pooled qwen3.6-35B)."""
...
class TransformerBlock(FFNBlock):
  ...
  def _attention(self, x:Tensor, start_pos:int|UOp) -> Tensor:
    ...
    mask = causal_mask(positions(self.config.max_context, x.device), T, start_pos, x.dtype, self.config.sliding_window) \
  ...
  def _init_state(self, x:Tensor):
    if not hasattr(self, "cache_kv"):
      positions(self.config.max_context, x.device)  # realize the shared position buffer here, outside any JIT capture (see causal_mask)
...
class MLATransformerBlock(FFNBlock):
  ...
  def _attention(self, x:Tensor, start_pos:int|UOp) -> Tensor:
    ...
    mask = causal_mask(positions(self.config.max_context, x.device), T, start_pos, x.dtype) if resolve(T != 1) else None
  ...
  def _init_state(self, x:Tensor):
    if not hasattr(self, "cache_k"):
      positions(self.config.max_context, x.device)
