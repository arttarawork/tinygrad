...
4. **Fused attention** — BOTH tracks dead-ended 2026-08-18; see T1.8b + T1.7 reports. (b) `custom_kernel`: no symbolic shapes (JIT makes Tk symbolic after token 1), BEAM can't search it, no warp primitives (~5% bw ceiling). (a) PCONTIG: **numerically incorrect** on multi-pass reduces (its own tests are self-referential via an SCACHE cache-key bug — T4.9), threadgroup crash at real GQA shapes, and symbolic Tk sizes on-chip buffers by the Variable's static upper bound (compile crash). Full evidence: `PCONTIG_ATTN_NOTES.md` on `task/T1.7-pcontig-attn`. **Current route: live with the correct 4-kernel chain (already ~46% of bandwidth at Tk=8k — the prize was only ~2x) and pursue the upstream enablers.** UPDATE 2026-08-19 (final pre-dock state): T4.7 + T1.8c landed — the custom kernel now runs EVERY decode token with symbolic Tk (plus a compound-expression fix in `to_kernel_param`), byte-identical output, at a ~2-3% loss vs the SDPA chain on llama3.2:1b. **T4.8's warp-reduce: FINAL NO-GO pre-dock** (bench window 3: FAST_ATTN a wash at p512 on qwen3:8b, +2% single-run at 4k ctx — not fundable at ~300-550 lines). Fused attention on Metal is closed until the dock; the machinery (hook + symbolic kernel) is built, correct, and waiting for sm_86 tensor cores (T2.4). T4.9 (SCACHE key) landed and un-masked the PCONTIG tests honestly. Net: the fused-attention machinery is fully built and correct — it awaits either warp primitives or the 3090's tensor cores (T2.4/dock) to pay off.
...
**Stage A — pipeline split (practical, sidesteps all three ⛔s).** PROVEN HOMOGENEOUS (T3.1) and **CROSS-BACKEND** 2026-08-18 (`task/T3.2-metal-cpu`): METAL+CPU split produces identical tokens (tiny configs + real qwen3:8b), JIT capture succeeds with METAL graph islands + sequential CPU + standalone boundary COPYs (COPY is structurally ungraphable, `engine/jit.py:151-152`). Two measured planning numbers: cross-backend copies cost a **fixed ~750 µs** below ~1M elements (overhead-, not bandwidth-dominated — budget per *hop*, not per byte; METAL↔NV over the socket will differ, re-measure post-dock), and lazily-placed weights get their placement COPYs captured into the JIT (fixed: `from_gguf` force-realizes moved params). Don't shard tensors; place *layers*. Each kernel stays single-device; only activations cross (a few KB/token — trivial even over a socket). Work: a device-map API in `tinygrad/llm` (`--device-map`, auto-split by free memory), per-layer KV cache device, weight loading to mapped devices (loader honors pre-placed params, `nn/state.py:211-214`), and verifying `TinyJit` captures a mixed-device trace with per-backend graphs and eager copies at boundaries. The `@function` per-block seam (`model.py:145-151`) is where to cut. **MoE placement policy** is sub-layer pipeline: attention + norms + KV on NV, routed-expert FFN tensors on METAL — BUILT 2026-08-18 (`task/T3.3-moe-placement`, `experts:<dev>` device_map segment; olmoe tokens exact METAL+CPU). Two measured corrections to this paragraph's assumptions: (1) it's **3 copies per MoE layer, not 2** — the selection indices travel with the activations for the expert gather; budget 3× the per-hop floor. (2) **The GGUF must load on the big-memory (expert) side, with device_map moving the small attention share** — moving the big expert tensors across a device boundary forces full-fp16 materialization at load (loses fused dequant, ~2x memory on exactly the tensors that don't fit). For the NV+METAL flagship: load on METAL (experts stay), map attention+KV to NV.
...
**Stage B — copy quality. RE-SCOPED 2026-08-18 (T3.4 spike, hypothesis refuted):** memory aliasing via `external_ptr` works (METAL-owns/CPU-borrows direction only — Metal's `external_ptr` takes an ObjC MTLBuffer id, not a raw pointer) but buys **nothing**: the measured fixed per-hop cost is synchronization (~150 µs+ CPU-blocking `waitUntilCompleted` full-queue drain after any dispatch), not the memcpy. Stage B is therefore the **async signal bridge** — PARKED FOR THE DOCK (T3.6 spike, 2026-08-19): a working race-tested MTLSharedEvent bridge LOSES 20-35 µs on METAL+CPU (CPU-producer sync is nearly free; eager Python dispatch ~300 µs dominates; MetalGraph caps the drain at K≈0-1 in JIT'd steady state). The capture-op design + sizing (~110-160 lines, 5 files) is in `SIGNAL_BRIDGE_NOTES.md` on `task/T3.6-signal-bridge`; revisit at TD.3 where the producer is NV over the socket and the round trip is orders of magnitude larger. Evidence trail: `task/T3.4-zero-copy` (aliasing refuted) → `task/T3.6-signal-bridge` (bridge refuted pre-dock).
...
| gpt-oss-20b | MXFP4 ~12 GB | NV alone | ✅ MEASURED (window 4): decode **10.97 tok/s no-BEAM / 15.52 BEAM** on M3 Pro Metal (was 1.69; T4.13 fix, 17x byte cut, 3.46 GB/token ≈ analytic). Long-context is attention-compute-bound → the 3090's tensor cores are the next lever for this model (T2.4/dock). |
