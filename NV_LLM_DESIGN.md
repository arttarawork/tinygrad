...
- ⚠️ **The batch-1 MATVEC heuristic misses the kernels that matter.** Its guard requires `MUL(INDEX, INDEX)` as the reduce body (`codegen/opt/heuristic.py:60-78`); fp16 and quantized gemvs wrap that in `CAST(...)`, so exactly the bandwidth-critical decode kernels fall through to generic scheduling. Needs on-hardware confirmation (`DEBUG=3` prints `MATVEC:`), then a small see-through-cast fix.
...
- ⚠️ MoE routing costs 4 extra tiny kernels/layer (`pairwise_topk`, `model.py:35-41`); full-vocab RNG runs even at temperature 0 (`model.py:364`).
