...
4. **Fused attention** — route order REVERSED by T1.8b's findings (2026-08-18): (b) the `custom_kernel` route is **blocked for production decode** — `Tensor.custom_kernel` asserts concrete shapes while the JIT makes Tk symbolic after the first token (kernel only ever fires once per generation), BEAM structurally can't search custom kernels (`opts_to_apply` bypasses it), and Metal codegen lacks simd_sum/shuffle warp primitives (~5% bw ceiling measured). The hook (T1.8) + tuned kernel (T1.8b, FAST_ATTN=1, default off) stay in-tree for when upstream lifts those limits. (a) **PCONTIG online-softmax (T1.7) is now the live route.** (c) is softened by a new datum: the existing 4-kernel chain already hits ~46% of bandwidth at Tk=8192 — the fused-attention win on Metal is nearer 2x than 5x. Upstream-shaped sub-levers that would unlock (b): symbolic-shape custom_kernel, warp-reduce primitives in the Metal renderer.
