...
4. **Fused attention** — BOTH tracks dead-ended 2026-08-18; see T1.8b + T1.7 reports. (b) `custom_kernel`: no symbolic shapes (JIT makes Tk symbolic after token 1), BEAM can't search it, no warp primitives (~5% bw ceiling). (a) PCONTIG: **numerically incorrect** on multi-pass reduces (its own tests are self-referential via an SCACHE cache-key bug — T4.9), threadgroup crash at real GQA shapes, and symbolic Tk sizes on-chip buffers by the Variable's static upper bound (compile crash). Full evidence: `PCONTIG_ATTN_NOTES.md` on `task/T1.7-pcontig-attn`. **Current route: live with the correct 4-kernel chain (already ~46% of bandwidth at Tk=8k — the prize was only ~2x) and pursue the upstream enablers.** UPDATE 2026-08-19: T4.7 landed — `custom_kernel` now takes symbolic dims (~20-line lowering fix), removing blocker (1); remaining to reopen the custom route: T1.8c (rewrite the tuned kernel's chunking kernel-side so it accepts symbolic Tk) and, if that kernel then needs cross-lane speed, T4.8's warp-reduce work (scoped: ~300-550 lines, framework-level, start from the >32 case). T4.9 (SCACHE key) landed and un-masked the PCONTIG tests honestly. The T1.8 hook + T1.8b kernel stay in-tree, default off.
