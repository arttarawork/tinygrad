...
4. **Fused attention** — BOTH tracks dead-ended 2026-08-18; see T1.8b + T1.7 reports. (b) `custom_kernel`: no symbolic shapes (JIT makes Tk symbolic after token 1), BEAM can't search it, no warp primitives (~5% bw ceiling). (a) PCONTIG: **numerically incorrect** on multi-pass reduces (its own tests are self-referential via an SCACHE cache-key bug — T4.9), threadgroup crash at real GQA shapes, and symbolic Tk sizes on-chip buffers by the Variable's static upper bound (compile crash). Full evidence: `PCONTIG_ATTN_NOTES.md` on `task/T1.7-pcontig-attn`. **Current route: live with the correct 4-kernel chain (already ~46% of bandwidth at Tk=8k — the prize was only ~2x) and pursue the upstream enablers (T4.7 symbolic custom_kernel, T4.8 Metal warp primitives, T4.9 SCACHE key) that would reopen a fused path.** The T1.8 hook + T1.8b kernel stay in-tree, default off, ready for that day.
