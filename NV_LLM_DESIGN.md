...
**Stage B — copy quality. RE-SCOPED 2026-08-18 (T3.4 spike, hypothesis refuted):** memory aliasing via `external_ptr` works (METAL-owns/CPU-borrows direction only — Metal's `external_ptr` takes an ObjC MTLBuffer id, not a raw pointer) but buys **nothing**: the measured fixed per-hop cost is synchronization (~150 µs+ CPU-blocking `waitUntilCompleted` full-queue drain after any dispatch), not the memcpy. Stage B is therefore the **async signal bridge** (T3.6): `MTLSharedEvent` waitForEvent encoded ahead-of-submit in the consumer + a watcher that signals it when the producer's HCQ signal word crosses the value — converting the drain into a GPU-side dependency edge. Needs a JIT-capturable foreign-wait op. Evidence + sync-semantics tests: `task/T3.4-zero-copy` (unmerged, kept as reference).
